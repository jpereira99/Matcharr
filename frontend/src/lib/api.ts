...
export const api = {
  ...
  testDispatcharr: (dispatcharr_url?: string, dispatcharr_token?: string) =>
    req<{ ok: boolean; message: string; detail?: Record<string, unknown> | null }>("/settings/test-dispatcharr", {
      method: "POST",
      body: JSON.stringify({ dispatcharr_url, dispatcharr_token }),
    }),

  listProfiles: () => req<LeagueProfile[]>("/profiles"),
  createProfile: (body: Partial<LeagueProfile> & { name: string; stream_pattern: string; espn_sport: string; espn_league: string }) =>
    req<LeagueProfile>("/profiles", { method: "POST", body: JSON.stringify(body) }),
  updateProfile: (id: number, body: Partial<LeagueProfile>) =>
    req<LeagueProfile>(`/profiles/${id}`, { method: "PUT", body: JSON.stringify(body) }),
  deleteProfile: (id: number) => req(`/profiles/${id}`, { method: "DELETE" }),
  testPattern: (pattern: string, stream_name: string) =>
    req<{ matched: boolean; groups: Record<string, string>; error?: string }>("/profiles/test-pattern", {
      method: "POST",
  ...
  createTeamChannel: (body: Omit<TeamChannel, "id" | "created_at">) =>
    req<TeamChannel>("/team-channels", { method: "POST", body: JSON.stringify(body) }),
  updateTeamChannel: (id: number, body: Partial<TeamChannel>) =>
    req<TeamChannel>(`/team-channels/${id}`, { method: "PUT", body: JSON.stringify(body) }),
  deleteTeamChannel: (id: number) => req(`/team-channels/${id}`, { method: "DELETE" }),

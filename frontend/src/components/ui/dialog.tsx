...
export function Dialog({ open, onClose, title, children, className, variant = 'center' }: DialogProps) {
	...
	return (
		<div className='fixed inset-0 z-50 flex items-center justify-center p-4' role='dialog' aria-modal='true'>
			<button type='button' className='absolute inset-0 bg-black/60 backdrop-blur-sm' aria-label='Close dialog' onClick={onClose} />
			<div
				className={cn(
			...
			>
				<div className='flex items-center justify-between'>
					<h2 className='text-lg font-extrabold tracking-tight font-heading'>{title}</h2>
					<button
						type='button'
						onClick={onClose}

	...
	return (
		<div className='space-y-6'>
			<header>
				...
				<p className='mt-1 text-sm text-(--color-muted)'>Configure Dispatcharr connection and scheduler behavior.</p>
			</header>
						...
						<span className='flex items-center gap-1.5 text-sm'>
							{test.data.ok ? (
								<>
									...
									<span className='text-(--color-success)'>{test.data.message}</span>
								</>
							) : (
								<>
									<XCircle className='h-4 w-4 text-(--color-danger)' />
									<span className='text-(--color-danger)'>{test.data.message}</span>
								</>
							)}
						</span>
					)}
			...
			<Card>
				...
				<div className='mt-4 grid gap-4 md:grid-cols-2'>
					<div>
						...
						/>
						<p className='mt-1 text-xs text-(--color-muted)'>How often the scheduler checks for games to route.</p>
					</div>
					<div>
						...
						/>
						<p className='mt-1 text-xs text-(--color-muted)'>Start routing this many minutes before game time.</p>
					</div>
					<div>
						...
						<Input
							...
							onChange={(e) => update({ schedule_lookahead_days: +e.target.value })}
						/>
						<p className='mt-1 text-xs text-(--color-muted)'>How many days ahead to fetch from ESPN.</p>
					</div>
					<div>
						...
						<Input
							...
							onChange={(e) => update({ schedule_refresh_hours: +e.target.value })}
						/>
						<p className='mt-1 text-xs text-(--color-muted)'>Minimum hours between ESPN schedule re-fetches.</p>
					</div>
				</div>
			</Card>
			...
			<Card>
				...
				<div className='mt-4 grid gap-4 md:grid-cols-2'>
					<div>
						...
						/>
						<p className='mt-1 text-xs text-(--color-muted)'>IANA timezone for schedule display.</p>
					</div>
					<div>
						<Label>Theme</Label>
						<div className='mt-1'>
			...
			<div className='sticky bottom-0 -mx-5 border-t border-(--color-border) bg-(--color-background)/95 px-5 py-3 backdrop-blur-sm md:-mx-8 md:px-8 lg:-mx-10 lg:px-10'>
				<div className='mx-auto flex max-w-6xl items-center justify-between'>
					<span className='text-sm text-(--color-muted)'>{dirty ? 'You have unsaved changes.' : 'All changes saved.'}</span>
					<Button type='button' size='sm' disabled={!dirty || save.isPending} onClick={() => form && save.mutate(form)}>

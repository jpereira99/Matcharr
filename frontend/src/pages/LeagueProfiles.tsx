...
export function LeagueProfilesPage() {
	...
	const update = useMutation({
		mutationFn: ({ id, ...body }: Partial<LeagueProfile> & { id: number }) => api.updateProfile(id, body),
		onSuccess: () => {
			void qc.invalidateQueries({ queryKey: ['profiles'] });
			setEditId(null);
		},
	...
	const toggleEnabled = useMutation({
		mutationFn: ({ id, enabled }: { id: number; enabled: boolean }) => api.updateProfile(id, { enabled }),
		onSuccess: () => void qc.invalidateQueries({ queryKey: ['profiles'] }),
	});
	...
	return (
		<div className='space-y-6'>
			<header className='flex flex-col gap-4 md:flex-row md:items-end md:justify-between'>
				<div>
					<h1 className='text-2xl font-extrabold tracking-tight font-heading'>League Profiles</h1>
					<p className='mt-1 text-sm text-(--color-muted)'>Define stream patterns and ESPN league mappings.</p>
				</div>
					...
					>
						...
						<div>
							...
							<Input
								...
								onChange={(e) => setForm({ ...form, stream_name_filter: e.target.value })}
						...
						<EspnLeaguePick
							...
							onChange={(v) => setForm({ ...form, espn_sport: v.espn_sport, espn_league: v.espn_league })}
						/>
						<StreamPatternField value={form.stream_pattern} onChange={(v) => setForm({ ...form, stream_pattern: v })} />
					...
					{q.data?.map((p) => (
						<Card key={p.id} className='flex flex-col'>
							<div className='flex items-start justify-between gap-3'>
								...
								<Toggle
									checked={p.enabled}
									onChange={(v) => toggleEnabled.mutate({ id: p.id, enabled: v })}
			...
			{/* Edit dialog */}
			<Dialog open={editId !== null} onClose={() => setEditId(null)} title='Edit Profile' variant='panel'>
				<form
				...
				>
					...
					<div>
						...
						<Input
							...
							onChange={(e) => setEditForm({ ...editForm, stream_name_filter: e.target.value })}
